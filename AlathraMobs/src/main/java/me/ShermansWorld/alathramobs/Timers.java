...
public class Timers {
	...
	private BukkitTask giantSquidTimer;
	private ArrayList<Player> validGiantSquidSpawnpointPlayers = new ArrayList<Player>();
	...
	public void startGiantSquidTimer() {
		giantSquidTimer = new BukkitRunnable() {
			public void run() {
				validGiantSquidSpawnpointPlayers.clear();
				for (Player p : Bukkit.getOnlinePlayers()) {
					if (p.getWorld().getName().contentEquals("World-o")) {
						if (BiomeUtil.getGiantSquidBiomes().contains(BiomeUtil.getBiomeName(p))
								&& !TownyUtil.isLocationInTown(p.getLocation())) {
							if (!Util.isLocationUnderground(p.getLocation())) {
								validGiantSquidSpawnpointPlayers.add(p);
							}
						}
					}
				}

				if (validGiantSquidSpawnpointPlayers.isEmpty()) {
					return;
				}

				int randIdx = rand.nextInt(validGiantSquidSpawnpointPlayers.size());
				MobsUtil.spawnWaterMobNearPlayer(10, validGiantSquidSpawnpointPlayers.get(randIdx), "MSO_GiantSquid");

			}
		}.runTaskTimer(AlathraMobs.getInstance(), 0L, Config.giantSquidTimerInterval * 20);

	}

	public void stopGiantSquidTimer() {
		giantSquidTimer.cancel();
	}
